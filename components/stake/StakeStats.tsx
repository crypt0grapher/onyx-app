...
const useTCommon = () => useTranslations("common");
...
const StakeStats: React.FC = () => {
    ...
    const handleClaimRewards = async () => {
        if (!isConnected) {
            showToast({
                variant: "danger",
                text: tc("errors.wallet.walletDisconnected", {
                    default: "Wallet Not Connected",
                }),
                subtext: tc("errors.wallet.notFoundDescription", {
                    default: "Please connect your wallet to claim rewards",
                }),
            });
            return;
        }
        ...
        if (pendingRewardsRaw <= 0) {
            showToast({
                variant: "danger",
                text: t("noRewards", { default: "No Rewards" }),
                subtext: t("noRewardsSubtext", {
                    default: "You have no rewards to claim",
                }),
            });
            return;
        }
        ...
        try {
            await executeTransaction({
                action: () =>
                    claim().then((r) => r as unknown as `0x${string}`),
                successText: tt("staking.claimSuccess", {
                    default: "Rewards claimed successfully!",
                }),
                errorText: tt("staking.claimFailed", {
                    default: "Failed to claim rewards",
                }),
                onSuccess: () => {
                    refetch();
